<template>
    <div class="brand-form-container">
        <h2 class="form-title">Chỉnh sửa thương hiệu</h2>
        <form @submit.prevent="updateBrand">
            ...
            <div class="button-group">
                <button type="submit" class="action-button submit-button">Cập nhật</button>
...
export default {
    ...
    methods: {
        async fetchBrand() {
            try {
                const response = await fetch(`http://localhost:5000/api/brands/${this.brandId}`);
                if (!response.ok) throw new Error("Lỗi khi tải thông tin thương hiệu.");
        ...
        async updateBrand() {
            try {
                ...
                if (!response.ok) throw new Error("Lỗi khi cập nhật thương hiệu.");
